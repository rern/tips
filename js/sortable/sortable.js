...
usage: 
	...
	$('tableid').sortable({
		divBeforeTable: 'divid',	// default: (none) - div above table, enclosed in single div
		divAfterTable: 'divid',	// default: (none) - div after table, enclosed in single div
...
$.fn.sortable = function(options) {
	...
	var $thtd = $thead.find('td');
	var $tbody = $table.find('tbody');
	var $tbtr = $tbody.find('tr');
	
	var tblid = this[0].id;
	var tblparent = '#sortable'+ tblid;
	var beforetblH = settings.divBeforeTable ? $(settings.divBeforeTable).outerHeight() : 0;
	...
	// dynamic css - for beforetblH underlay and fixed thead2
	$('head').append('<style>'+
		tblparent +'::before {'+
			'content: "";'+
			'display: block;'+
			'height: '+ (beforetblH + 5) +'px;'+ // +5px offset 1st row
		'}\n'+
		'@media(max-height: '+ shortvport +'px) {'+
			tblparent +'::before {display: block; height: 0;}'+
		'}\n'+
		
		'.sortableth2 {top: '+ beforetblH +'px;}\n'+
		'.sortableth2 a {padding: '+ $table.find('td').css('padding') +';}\n'+
		'@media(max-height: '+ shortvport +'px) {'+
			settings.divBeforeTable +' {position: relative;}'+
			'.sortableth2 {top: 0;}'+
...
//		scrltop = $window.scrollTop(); // !!! detect incorrectly in fullscreen ios, chrome devtools
		...
		if ($window.height() < shortvport) {
			var scrltop0 = scrltop + beforetblH;
			fromshortv = 1;
		} else {
			var scrltop0 = scrltop - (fromshortv ? beforetblH : 0); // calc if from short viewport
			fromshortv = 0;
		}
		$thead2.hide();
